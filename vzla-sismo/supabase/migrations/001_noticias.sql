...
create table if not exists noticias (
  ...
  credibilidad_fuente text not null default 'media'
    check (credibilidad_fuente in ('maxima', 'alta', 'media', 'baja')),
  credibilidad_score int not null default 50 check (credibilidad_score between 0 and 100),
  ...
  -- Veracidad combinada (fuente + análisis)
  veracidad text not null default 'no_verificado'
    check (veracidad in ('confirmado', 'probable', 'no_verificado', 'dudoso')),
  veracidad_score int check (veracidad_score between 0 and 100),

  publicado_at timestamptz,
  creado_at timestamptz default now()
);

-- RLS: solo noticias aprobadas son públicas
